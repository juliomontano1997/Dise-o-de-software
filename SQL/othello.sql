...
$body$
BEGIN
	IF ((SELECT COUNT(playerID) FROM players WHERE mail = i_mail) > 0) 
		THEN
			IF ((SELECT image FROM players WHERE mail = i_mail) != i_image) 
				THEN
	...
	RETURN query (SELECT playerID, playerLevel FROM players WHERE mail = i_mail);
...
BEGIN
	RETURN query
		SELECT playerOneID, playerTwoID, actualPlayerID, boardSize, board, colorPlayer1, colorPlayer2, amountPassTurn 
		FROM sessions where sessionID = i_sessionID;
END;
$body$
LANGUAGE plpgsql;
...
$body$
BEGIN
	IF i_actualPlayerID = (SELECT actualPlayerID FROM sessions WHERE sessionID = i_sessionID) 
	THEN
		UPDATE sessions SET board = i_board WHERE sessionID = i_sessionID;
		IF i_actualPlayerID = (SELECT playerOneID FROM sessions WHERE sessionID = i_sessionID) 
			THEN 
				UPDATE sessions SET actualPlayerID = (SELECT playerTwoID FROM sessions WHERE sessionID = i_sessionID)
...
$body$
BEGIN
	IF (SELECT winsPlayer1 FROM sessionStadistics WHERE sessionID = i_sessionID) 
		>(SELECT winsPlayer2 FROM sessionStadistics WHERE sessionID = i_sessionID) 
		THEN
			UPDATE players SET playerLevel = playerLevel + 1 
			WHERE playerID = (SELECT playerOneID FROM sessions WHERE sessionID = i_sessionID);

	ELSIF (SELECT winsPlayer1 FROM sessionStadistics WHERE sessionID = i_sessionID) 
		< (SELECT winsPlayer2 FROM sessionStadistics WHERE sessionID = i_sessionID) 
		THEN
			UPDATE players SET playerLevel = playerLevel + 1 
			WHERE playerID = (SELECT playerTwoID FROM sessions WHERE sessionID = i_sessionID);
	ELSE
		UPDATE players SET playerLevel = playerLevel + 1 
			WHERE playerID = (SELECT playerOneID FROM sessions WHERE sessionID = i_sessionID);
		UPDATE players SET playerLevel = playerLevel + 1 
	...
	UPDATE sessions SET active=FALSE WHERE sessionID = i_sessionID;
	RETURN TRUE;
	EXCEPTION WHEN OTHERS THEN RETURN FALSE;
END;
$body$
...
BEGIN
	...
	RETURN query
		SELECT 
			...
			sessionData.amountGames, 
			sessionData.numberActualGame, 
			p.playerName, 
			sessionData.board 
		...
		FROM 
			(SELECT 
				...
				(SELECT 
					...
					s.playerTwoId 
					FROM sessions s
					WHERE  active = TRUE AND (playerOneID = i_playerID OR playerTwoID = i_playerID)	
				) AS s 
				INNER JOIN 
				sessionStadistics AS t
		...
		players AS p 
		ON ((p.playerID = sessionData.playerOneId AND p.playerID != i_playerID) 
		   OR 
		   (p.playerID = sessionData.playerTwoId AND p.playerID != i_playerID));
END
$body$
LANGUAGE plpgsql;
...
BEGIN
SELECT count(*) INTO sessionExists FROM sessions WHERE active=TRUE (playerOneID = i_transmitterID AND playerTwoID = i_receiverID) OR (playerOneID = i_receiverID AND playerTwoID = i_transmitterID);
...
newSessionID INT;
BEGIN
sessionExists := (SELECT count(*) FROM sessions WHERE active = TRUE  AND playerOneID = i_playerID AND playerTwoID = 0);
...
DEMO player
*********************************/
INSERT INTO players VALUES(0, 'PlayerDemo@gmail.com','Machine',0,'../../assets/images/machine.jpg',FALSE);
